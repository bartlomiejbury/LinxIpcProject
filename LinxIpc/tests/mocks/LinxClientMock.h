...
class LinxClientMock : public LinxClient {
  public:
    ...
    MOCK_METHOD(int, send, (const LinxMessage &message), (override));
    MOCK_METHOD(LinxMessagePtr, receive, (int timeoutMs, const std::vector<uint32_t> &sigsel), (override));
    MOCK_METHOD(LinxMessagePtr, sendReceive, (const LinxMessage &message, int timeoutMs, const std::vector<uint32_t> &sigsel), (override));
