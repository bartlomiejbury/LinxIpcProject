...
TEST_F(LinxIpcServerTests, getPollFdCallQueueGetFd) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
...
TEST_F(LinxIpcServerTests, sendReturnSocketSendResult) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
    auto msg = LinxMessageIpc(10);
    auto client = std::make_shared<NiceMock<LinxIpcClientMock>>();
...
TEST_F(LinxIpcServerTests, endpoint_createClientCallEndpointCreateCLient) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
...
TEST_F(LinxIpcServerTests, start_CallStartNewThread) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
...
TEST_F(LinxIpcServerTests, stop_DoNothingWhenNotStarted) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
...
TEST_F(LinxIpcServerTests, stop_CallThreadCancel) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
    endpoint->start();
...
TEST_F(LinxIpcServerTests, thread_NotAddToQueueWhenReceiveReturnNull) {
    auto endpoint = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
    endpoint->start();
...
TEST_F(LinxIpcServerTests, handleMessage_CallQueueGet) {
    auto server = std::make_shared<LinxIpcServerImpl>(endpointMock, queueMock);
